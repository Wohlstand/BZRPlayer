...
static FMOD_RESULT F_CALL open(FMOD_CODEC_STATE *codec, FMOD_MODE usermode, FMOD_CREATESOUNDEXINFO *userexinfo) {
    ...
    // for gm.dls only (mimics fmod defaults)
    ...
    //TODO load gm.dls from mem not path (thru new_fluid_dls_loader/new_fluid_defsfloader)
    // auto *dlsLoader = new_fluid_dls_loader(plugin->synth, settings);
    //
    // if (dlsLoader == nullptr) {
    //     delete plugin;
    //     return FMOD_ERR_FORMAT;
    // }
    ...
    if (const string gmDlsPath = plugin->info->dataPath + GM_DLS_PATH;
        fluid_synth_sfload(plugin->synth, gmDlsPath.c_str(), 1) == FLUID_FAILED) {
        delete plugin;
        return FMOD_ERR_FORMAT;
    }
